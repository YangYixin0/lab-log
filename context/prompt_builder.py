...
### 1. 事件识别
- 识别视频中的人物动作、设备操作、设备状态变化
- 根据视频画面左上角的时间戳水印确定时间
- **event_type 只能是 "person" 或 "equipment-only"**，不能使用其他值
- 多个人物同时出现时，如果人物动作关系密切，则记录为多人共同参与的事件（person_ids 包含多个人物编号），否则分开记录各自的事件。
- 设备显示数值且与人物动作无关时，单独记录为 equipment-only 事件（person_ids 为空数组）
...
- **稀有特征**一致是合并的强信号，但仍需确认无冲突
- 若缺少稀有特征区分，不要轻易合并已有的外貌记录
- 发现全新人物时，追加新记录
...
{{
  "events_to_append": [
    ...
    {{
      "event_id": "evt_00043",
      "start_time": "2025-12-24T10:00:15",
...
- equipment-only 事件的 person_ids 应为空数组 []
- 如果视频中没有新事件，events_to_append 返回空数组
