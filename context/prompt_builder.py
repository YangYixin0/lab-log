...
- 关注设备显示的数值。如果设备示数与人物动作无关，那么单独记录为 equipment-only 事件（person_ids 为空数组）
- 不同事件的时间范围可以有部分或完全重叠，例如，如果画面中同时出现某个人物和某个显示数值的设备，而且人物动作和设备状态变化是独立的，那么这两个事件应当分开记录而且有部分或完全的重叠时间范围。
- 关注人物使用的化学品。如果能看清名称则记录名称，否则记录化学品的颜色、物质状态。
- 描述人物把某个物品从某个位置取出或放到某个位置，例如抽屉，如果画面内有多个抽屉，那么应当借助画面中有唯一性、一般不会转移的物品和方位词来辅助描述确定的抽屉对象。
