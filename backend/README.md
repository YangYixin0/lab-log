...
### `WebSocketServer` 类 (`server.py`)
主服务器类，负责：
- WebSocket连接管理（Arduino和前端客户端）
- 客户端连接路由（根据路径分发到不同处理函数）
- 协调 `ImageManager` 和 `FPSMonitor`
- 命令转发和日志发送

### `ImageManager` 类 (`image_manager.py`)
图片管理类，负责：
- 图片存储和文件命名
- 帧序号管理
- 自动清理旧图片（保持最多200张）

### `FPSMonitor` 类 (`fps_monitor.py`)
FPS统计类，负责：
- 帧计数统计
- FPS计算（每秒更新）
- 统计重置
